...
import { getAllSponsorSureTuru, getAllPublishedSponsorSureTuru, deleteSponsorSureTuru, deleteAllSponsorSureTuru } from '../../../../redux-new/actions/sponsorSureTuru';
import Modal from 'react-modal';
import { ToastContainer, toast } from 'react-toastify';
import AuthService from "../../../../services-socket/auth.service";
import '../../../responsive-table.css';
import CreateSponsorSureTuru from './CreateSponsorSureTuru';
import UpdateSponsorSureTuru from './UpdateSponsorSureTuru';
...
export default function DashboardGetAllSponsorSuresi() {
    ...
    // Fonksiyon tanımlamaları
     const getData =  async () => {
         await dispatch(getAllSponsorSureTuru()); 
    }

    const publishedGetData = async () => {
        await dispatch(getAllPublishedSponsorSureTuru());
    }

    const deleteSponsorSureTuruFunc = (id) => {
     if (window.confirm('Gerçekten bu veriyi silmek istiyor musunuz?')) {
            notify2();
            setTimeout(() => {
                dispatch(deleteSponsorSureTuru({ id }));
    ...
    const deleteAllSponsorSureTuruFunc = () => {
        if (window.confirm('Gerçekten tüm verileri silmek istiyor musunuz?')) {
            notify2();
            setTimeout(() => {
                dispatch(deleteAllSponsorSureTuru());
                window.location.reload(false);
            }, 4500);
          } else {
            return false;
        ...
        <div className='dashboard-container'>
            ...
            <div className='dashboard-body'>
                <div>
                    ...
                    >
                        <h2 ref={(_subtitle) => (subtitle = _subtitle)}>Yeni Sponsor Süre Türü Ekle</h2>
                        <CreateSponsorSureTuru close={closeCreateModal}/>
                    </Modal>
                </div>
                <div>
                    <Modal
                        ...
                        contentLabel="Example Modal"
                    >
                        <h2 ref={(_subtitle) => (subtitle = _subtitle)}>Sponsor Süre Türünü Düzenle</h2>
                        <UpdateSponsorSureTuru close={closeUpdateModal} id={gonderId} />
                    </Modal>
                </div>
                ...
                <div className="listele-div">
                    ...
                    <button onClick={() => deleteAllSponsorSureTuruFunc()} className="listele-button listele-button-danger">Tümünü Sil</button>
                </div>
                <div style={{ marginTop: '30px', marginLeft: '10px', marginRight: '10px' }}>
                        <table className="fixResponsiveTable">
                            ...
                            <tbody>
                            {
                                sponsorSureTuru.length > 0
                                ? sponsorSureTuru.map((item, index) => {
                                    ...
                                    return (
                                        <tr>
                                            ...
                                            <td data-label="Sil"><button onClick={() => deleteSponsorSureTuruFunc(item.id)} className="listele-button listele-button-danger">Sil</button></td>
                                        </tr>
                                    );
                                })
