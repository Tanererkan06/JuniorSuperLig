...
function UpdateOyun(props) {
    ...
    const oyuncularFunction1 = (value) => {
      ...
      const data = { takimbiresamelistesi: value};
      dispatch(updateOyun({id: props.id, data}));
    }

    const oyuncularFunction2 = (value) => {
      setTakimIkiEsameListesi(value);
      const data = { takimikiesamelistesi: value};
      dispatch(updateOyun({id: props.id, data}));
    }
    ...
    const updateOyunFunc = () => {
      if(takimBirAdi != takimIkiAdi)
      {
        ...
        const data = {adi, mactarihi: macTarihi, macsaati: macSaati, lig, sehir, yer, takimbirid: takimBirId, takimbiradi: takimBirAdi, takimbiresamelistesi: takimBirEsameListesi, takimikiesamelistesi: takimIkiEsameListesi, takimikid: takimIkiId, takimikiadi: takimIkiAdi, takimbiresamelistesi: takimBirEsameListesi, takimikiesamelistesi: takimIkiEsameListesi, gozlemciid: gozlemciId, gozlemciadi: gozlemciAdi, yorum, macsonu: macSonu, ilkyarisonucu: ilkYariSonucu, published}
          dispatch(updateOyun({id: props.id, data}));
          notify();
          setTimeout(() => {
            props.close();
