...
function UpdateTakim(props) {
  ...
  useEffect(() => {
    ...
    TakimService.getById(id).then(response => {
      // //console.log("TAKIM VERI: ", response.data);
  ...
  const updateTakimFunc = () => {
    // const data = {adi, kurulustarihi: kurulusTarihi, telefon, adres, oyunKonum, fax, website, oyuncular, hocaId, hocaAdi, logo, konum, lig, sehir, sorumlu, sorumluIletisim, eposta, sifre, published}
    const data = { adi, kurulustarihi: kurulusTarihi, telefon, adres, oyunKonum, fax, website, oyuncular, hocaId, hocaAdi, logo, konum, lig, sehir, sorumlu, sorumluIletisim, published }
