...
function UpdateOyun(props) {
    ...
    const takimBirFunction = (value) => {
      ...
      const data = { takimbirid: value.id, takimbiradi: value.adi};
    ...
    const takimIkiFunction = (value) => {
      ...
      const data = { takimikiid: value.id, takimikiadi: value.adi};
      dispatch(updateOyun({id: props.id, data}));
    }
    ...
    const gozlemciFunction = async (value) => {
      ...
      const data = { gozlemciid: value.id, gozlemciadi: value.adi};
      dispatch(updateOyun({id: props.id, data}));
    }
    ...
    const updateOyunFunc = () => {
      ...
      {
        const data = {adi, macTarihi, macSaati, lig, il, yer, takimBirId, takimBirAdi, takimIkiId, takimIkiAdi, gozlemciId, gozlemciAdi, ortaHakem, yanHakemBir, yanHakemIki, dorduncuHakem, yorum, macsonu: macSonu, ilkyarisonucu: ilkYariSonucu, published}
          dispatch(updateOyun({id: props.id, data}));
          notify();
          setTimeout(() => {
            props.close();
        ...
        <form className="formcss">
          ...
          <div className="col-xs-12" style={{ marginLeft: '-30px' }}>
            <div className='col-xs-4'>
                ...
                <input className="inputcss" type="text" placeholder="İl" name="il" value={il} onChange={(event) => { setIl(event.target.value); }} />
            </div>
            <div className='col-xs-4'>
